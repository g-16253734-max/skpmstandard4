...
st.markdown("""
    <style>
    .stCheckbox { margin-bottom: 8px; padding-left: 20px; }
    .stSubheader { color: #1E88E5; border-bottom: 2px solid #1E88E5; padding-bottom: 5px; margin-top: 40px;}
    .section-title { font-weight: bold; background-color: #f0f2f6; padding: 8px 12px; border-radius: 5px; margin-top: 25px; display: block; border-left: 5px solid #1E88E5; }
    .spacer { margin-bottom: 30px; }
    </style>
    """, unsafe_allow_html=True)

# --- DATABASE KELAS (IKUT IMEJ) ---
KELAS = [
    "--- Pilih Kelas ---",
...
t3 = st.checkbox("iii. Mengikut ketetapan kurikulum", key="411aiii")
st.markdown('<div class="spacer"></div>', unsafe_allow_html=True)

st.markdown('<span class="section-title">4.1.1 (b) RPH: Menentukan kaedah pentaksiran</span>', unsafe_allow_html=True)
t4 = st.checkbox("i. Mengikut aras keupayaan murid (Pentaksiran)", key="411bi")
t5 = st.checkbox("ii. Mengikut peruntukan masa (Pentaksiran)", key="411bii")
t6 = st.checkbox("iii. Mengikut ketetapan kurikulum (Pentaksiran)", key="411biii")
st.markdown('<div class="spacer"></div>', unsafe_allow_html=True)

st.markdown('<span class="section-title">4.1.1 (c) RPH: Menyediakan sumber pendidikan (BBM/TMK)</span>', unsafe_allow_html=True)
t7 = st.checkbox("i. Mengikut aras keupayaan murid (Sumber)", key="411ci")
t8 = st.checkbox("ii. Mengikut peruntukan masa (Sumber)", key="411cii")
...
t12 = st.checkbox("iii. Mengikut aras keupayaan murid secara berterusan", key="421aiii")
st.markdown('<div class="spacer"></div>', unsafe_allow_html=True)

st.markdown('<span class="section-title">4.2.1 (b) Mengawal komunikasi murid</span>', unsafe_allow_html=True)
t13 = st.checkbox("i. Secara berhemah", key="421bi")
t14 = st.checkbox("ii. Secara menyeluruh", key="421bii")
t15 = st.checkbox("iii. Secara berterusan", key="421biii")
st.markdown('<div class="spacer"></div>', unsafe_allow_html=True)

st.markdown('<span class="section-title">4.2.2 (a) Mengawal suasana pembelajaran</span>', unsafe_allow_html=True)
t16 = st.checkbox("i. Mengurus susun atur murid", key="422ai")
t17 = st.checkbox("ii. Mewujudkan suasana yang menyeronokkan", key="422aii")
...
t20 = st.checkbox("ii. Mengikut aras keupayaan murid", key="431aii")
...
t23 = st.checkbox("ii. Memberi peluang murid memberi respon", key="441aii")
t24 = st.checkbox("iii. Secara berterusan & berkesan", key="441aiii")
st.markdown('<div class="spacer"></div>', unsafe_allow_html=True)
...
if st.button("🚀 JANA LAPORAN AI"):
    if guru_opt == "--- Pilih Nama Guru ---" or kls_opt == "--- Pilih Kelas ---" or kod_admin != "KINARUT2024":
        st.error("Pastikan semua maklumat dipilih dan Kod Admin betul!")
    else:
        try:
            ...
            Pelibatan Murid: {peratus}%.
            
            Format:
            1. RUMUSAN KEKUATAN.
            2. KELEMAHAN & PENAMBAHBAIKAN.
            3. CADANGAN INTERVENSI.
            Bahasa: Melayu Formal & Profesional.
            """
            ...
            st.error(f"Sila check API Key di Secrets. Error: {e}")
