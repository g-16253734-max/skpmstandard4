    ...
    if guru_opt == "--- Pilih Nama Guru ---" or kls_opt == "--- Pilih Kelas ---" or mod_opt == "--- Pilih Mod ---":
        ...
        if is_authorized:
            try:
                ...
                # TUKAR: Gunakan 'gemini-1.5-flash-latest' atau 'gemini-1.5-pro'
                # Ini adalah nama model paling terkini untuk 2026
                model = genai.GenerativeModel('gemini-1.5-flash-latest') 
                
                sum_text = f"Pencerapan {mod_opt} untuk {guru_opt}, Subjek {sub_opt}, Kelas {kls_opt}. Pelibatan: {peratus}%."
                
                prompt = f"""Sebagai pakar pencerapan SKPMg2, tulis laporan Standard 4 untuk: {sum_text}. 
                Sediakan: 1. Kekuatan, 2. Penambahbaikan, 3. Cadangan. Gunakan nada profesional dalam Bahasa Melayu."""
                ...
                # Jika masih ralat, kita paparkan bantuan troubleshoot
                st.error(f"Ralat API: {str(e)}")
                st.info("💡 Tip: Pastikan 'GEMINI_API_KEY' dalam Secrets Streamlit adalah betul. Jika ralat 404 berterusan, model Google mungkin sedang dikemas kini.")
